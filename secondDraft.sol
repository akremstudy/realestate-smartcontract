...
contract CourtHouse is ERC721Full, Ownable{
    ...
    struct Property {
        ...
        string uri;// uri that points to property's token file, ideally ipfs uri.
    }
    ...
    function tokenizeProperty(address payable _current_owner, string memory _locationAddress, string memory _property_type, uint _last_price, string memory uri) public onlyOwner returns(uint) {
        ...
        properties[token_id] =  Property(_current_owner, _locationAddress, token_id, _property_type, _last_price, STATE.not_forSale, uri);
        return token_id;
    }
    ...
    function dates(address _owner, uint _deposti_deadline, uint _closing_date, uint _deposit_basis_points, uint token_id) public {
        require(_owner == properties[token_id].current_owner, "you don't own this property");
        start_date = now;
    ...
    // @params _buyer Potential buyer wallet address, token_id Identifier for token/property that buyer is interested in, _uri Pointer for that property, report_uri URI that will store event emissions.
    ...
    function appraisal(address _appraiser, bool _appraisal_pass, uint token_id) public returns(bool){
        require(msg.sender != buyer || msg.sender != properties[token_id].current_owner || msg.sender != inspector, "you don't have permission");
    ...
    function inspection(address _inspector, bool _inspection_pass, uint token_id) public returns(bool){
        require(msg.sender != buyer || msg.sender != properties[token_id].current_owner || msg.sender != appraiser, "you don't have permission");
    ...
    function askingPrice_balance(uint token_id, string memory report_uri) public payable {
        ...
        safeTransferFrom(properties[token_id].current_owner, buyer, token_id);//add transfer token ownership
        properties[token_id].status = STATE.sold;
        properties[token_id].last_price = total_amount;
        properties[token_id].current_owner = buyer;
